...
On voit donc bien qu'une petite parti des états-unis détient la totalité du pouvoir électoral étant donné que les vote entre républicains (rouge) et les démocrate (bleu) sont plutôt égaux et qu'il manque donc le choix des swing states pour déterminer qui passera au pouvoir.
...
On à ici un exemple d'effet "spoiler" (fig. \ref{sim:spoilereffect}).
Cet effet montre que si il y a deux candidats du même parti comme par exemple le triangle et l'hexagone.
Ces deux derniers possède beaucoup plus de vote à eu deux que le carré mais pourtant le carré est gagnant de cette élection étant donné que que la majorité etant pour le triangle et l'hexagone se divise en deux pour chaque candidat, ce qui donne un résultat inferieur a celui du carré.
Ici le carré ne possède pas du tout la majorité des votes (seulement ~35\%) mais va gagner les élections.
C'est arrivé en 2000 quand Ralph Nader et Al Gore se sont partagé les votes ce qui a permis a George Bush de gagner.
